...
- Plan of record: `plan-v6/PLAN.md` (v6.1). **ALL PHASES DONE:** E1 `a79c162` · E2 `3333d8c` · E3a `4372e26` · E3b `b8588a4` · E4a `995c625` · E5 `af4181a`. Build = `python3 tools/build_index.py` then `python3 tools/build_app.py`; edit the JS in `app/template.html` (extracted from build_app). **Next: merge `taxonomy-tree` → `main`** (Tuo's call, D3). L1/L2 green; L3: tree=59, HOT=35, HTML 3.97→2.24 MB (toward the ~1 MB target — residual = vendor_intel, a later lazy-load lever). Open follow-ups: 87 unresolved intel FKs, E4b (sub vendors→slug), lazy vendor_intel.
