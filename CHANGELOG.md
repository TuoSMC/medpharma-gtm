...
- **The one live duplicate** — compute-class filtering existed twice (launcher chip **and** the `fCompute` More-filters select doing the identical filter). Cut `fCompute` from the drawer (kept as an inert state holder that `exploreCompute` writes); the richer launcher chip is now the sole compute filter.
- **Play is now the tree spine.** Deleted the launcher Play-chip row (a third way to reach a concept the tree now owns) and rebuilt the left tree as a **dedicated 4-level accordion** — `playTree()`: **Play → AI-driven / No-AI → category → 子市場**. This is exactly the nested-drawer shape requested. `playTree` is isolated from the shared `l1node`/`l2into`/`catRow`, so the other three view-by lenses (care area / compute type / who uses it) are untouched — verified.
  - **Single-open L1**: opening one Play collapses its siblings ("the tree reshapes into that Play").
  - **Filter-derived open-state** (the tree is rebuilt each render, so state is never persisted): default opens only the Play holding the selected/first category; any active filter/search auto-expands every surviving path.
  - **L4 子市場** rows carry their own opportunity/buyer; clicking one opens the parent's battle card and scrolls+flashes to that sub-market's card (`id=sub-<id>`, synchronous — no `requestAnimationFrame`, which pauses when the tab is backgrounded). Only PACS/VNA has 子市場 today (8), so L4 shows there; every other category is an L3 leaf until more land.
- **Default lens flipped** from care-area to the Play spine; buyer jumps + compute chips now land on the spine. Relabelled the `fSeg` filter "customer segment" to disambiguate it from the "who uses it" stakeholder lens.
- +5 `TestExploreSpine` guards (playTree defined+routed · the four dead selects + fSpans stay removed · fCompute not re-duplicated in the drawer · Play-chip row stays gone · subcard scroll anchor present). **137 tests green**; app=docs byte-identical; browser-verified all four lenses, single-open, L4 scroll+flash, buyer/compute/search landings; **0 console errors**; no emoji. Stays on `taxonomy-tree`; main keeps deploying v5.2.
