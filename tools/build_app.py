...
.byline{color:var(--ink);font-size:12.5px;font-weight:700;margin-top:4px;letter-spacing:.2px}
...
.ccchip{cursor:pointer;border:1px solid var(--line);border-radius:8px;padding:5px 9px;display:inline-flex;gap:6px;align-items:center;background:var(--panel)}
.ccchip:hover{border-color:var(--accent);background:var(--accentbg)}
.ccchip .dn{font-weight:700;color:var(--muted);font-size:11px}
...
function renderTaxonomy(){
  ...
  [['domain',T('care area','照護領域')],['compute',T('compute type','計算類型')],['stakeholder',T('who uses it','使用者')],
   ['ai',T('AI / no-AI','AI／非AI')],['play',T('play','打法')],['primary_buyer',T('who buys','誰買硬體')],['hardware_profile',T('hardware','硬體元件')]]
  ...
  function playBrief(pid){
    ...
    const cw0=el('div',{style:'display:flex;flex-wrap:wrap;gap:6px'});
  ...
  function detailCard(c){
    ...
    brow(T('Compute · SMCI box','計算 · SMCI 機箱'),el('span',{},
      el('span',{class:'pill ccpill '+ccClass(cck)},ccLabel(cck)),
      el('span',{class:'muted',style:'margin-left:7px'},ccSku(cck)+(ccNoGpu(cck)?' · '+T('no GPU','無 GPU'):' · '+T('GPU','GPU')))));
  ...
  function masterTable(){
    ...
    CC_ORDER.forEach(cc=>{const mem=cats.filter(c=>computeClassOf(c)===cc);if(!mem.length)return;
      ...
      chip.append(el('span',{class:'pill ccpill '+ccClass(cc)},ccLabel(cc)),
        el('span',{class:'dn'},'★'+flag+' · '+(LANG==='zh'?('直銷'+hot):(hot+' direct'))));
      chip.onclick=()=>window.exploreCompute(cc);crow.append(chip);});
    box.append(crow);
    box.append(el('div',{class:'muted',style:'font-size:11px;margin:2px 0 5px'},T('★ = flagship deals (biggest SMCI box) · direct = the end org buys its own iron. Deliberately NOT category count — that over-weights low-value buckets.','★ = 旗艦單(最大 SMCI 機箱)· 直銷 = 終端機構自購硬體。刻意不用類別數 — 類別數會高估低價值桶。')));
