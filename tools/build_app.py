...
function renderTaxonomy(){
  ...
  function render(){
    ...
    keys.forEach(k=>{
      ...
      const pl=gp==='play'?PLAY_BY[k]:null;
      if(pl&&pl.hardware_anchor&&pl.hardware_anchor.length)host.append(el('div',{class:'subhdr'},T('quote','報價')+': '+pl.hardware_anchor.join(' · ')));
      const grid=el('div',{class:'grid'});
      map.get(k).slice().sort((a,b)=>b.hardware_opportunity-a.hardware_opportunity||a.id.localeCompare(b.id)).forEach(c=>grid.append(card(c)));
      host.append(grid);
    });
  }
